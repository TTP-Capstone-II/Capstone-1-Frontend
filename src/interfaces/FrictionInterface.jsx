...
const FrictionInterface = ({ userInput, setUserInput, user, simulation }) => {
  ...
  const handlePatch = async () => {
    try {
      const response = await axios.patch(
        `${API_URL}/api/simulation/${simulation.id}`,
        {
          storedValues: userInput,
          forumTitle: forum,
          topic: "friction",
      ...
      if (target === "all") {
        ...
        targets.forEach((t) => {
          try {
            ...
            const formulaData = getFrictionFormulasForTarget(
              functionName,
              userInput,
              targetResult
            );
        ...
        return getFrictionFormulasForTarget(
          functionName,
          userInput,
          targetResult
        );
  ...
  useEffect(() => {
    ...
    try {
      ...
      const formulaData = generateFormulas(
        userInput.target,
        {
          mass: userInput.mass,
          gravity: userInput.gravity,
  ...
  return (
    <MathJaxContext config={mathJaxConfig}>
      ...
      >
        ...
        {results && (
          <Box
            sx={{ mt: 1, p: 2, backgroundColor: "#e3f2fd", borderRadius: 1 }}
          >
              ...
              <Box>
                {results.normalForce && (
                  <Typography variant="body2">
                    Normal Force: {Number(results.normalForce).toFixed(2)} N
                  </Typography>
                )}
                {results.frictionForce && (
                  <Typography variant="body2">
                    Friction Force: {Number(results.frictionForce).toFixed(2)} N
                  </Typography>
                )}
                {results.parallelForce && (
                  <Typography variant="body2">
                    Parallel Force: {Number(results.parallelForce).toFixed(2)} N
                  </Typography>
                )}
                {results.netForce && (
                  <Typography variant="body2">
                ...
                {results.distance && (
                  <Typography variant="body2">
                    Distance: {Number(results.distance).toFixed(2)} m
                  </Typography>
                )}
                {results.time && (
                  <Typography variant="body2">
                    Time: {Number(results.time).toFixed(2)} s
                  </Typography>
                )}
              </Box>
            ) : (
              <Typography variant="body2">
                {results[userInput.target] !== undefined
                  ? `${Number(results[userInput.target]).toFixed(2)} ${getUnit(
                      userInput.target
                    )}`
                  : "No result available"}
              </Typography>
